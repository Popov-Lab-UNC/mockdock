...
class FCGMBEvaluator:
    ...
    def compute_metrics(self, results_csv: Path, output_path: Optional[Path] = None) -> dict:
        ...
        # ─── Intrinsic Metrics ────────────────────────────────────────────────
        all_smiles = df["original_smiles"].to_list() if "original_smiles" in df.columns else df["smiles"].to_list()
        ...
        if not scored_df.is_empty():
            ...
            metrics["valid_pose_hit_rate"] = 0.0
...
def main():
    parser = argparse.ArgumentParser(description="FCGMB Evaluator CLI")
    ...
    parser.add_argument("--scratch-dir", type=Path, default=None, help="FCGMB scratch directory")
    
    args = parser.parse_args()
    
    evaluator = FCGMBEvaluator(args.benchmark, scratch_dir=args.scratch_dir)
