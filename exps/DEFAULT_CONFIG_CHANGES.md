...
### InVirtuoGen (`benchmark/exps/invirtuogen/`, `benchmark/exps_upperbound/invirtuogen/`)

- Integrated the full InVirtuoGen PMO optimization loop (crossover GeneticPrompter, SMILES GA mutations, PeakSeekerBandit, PPO/Reinforce updates).
- Subprocess Isolation: Because InVirtuoGen requires older rdkit (2023.9.6) and MockDock requires newer packages, the entire optimization loop runs inside a stateful subprocess in the InVirtuoGen environment.
- Local Socket Server: Added a host-side `OracleServer` TCP server in `run.py` to pipe candidate SMILES back to the host benchmark environment for docking.
- Anti-cheat enforcement:
  - Deactivated pre-filtering and deduplication in the optimizer loop.
  - Submits ALL generated SMILES (including duplicates, invalid SMILES, and fragment-constraint violations) in a batch to the host oracle for evaluation.
  - Submits GA mutations directly to the host oracle.
  - Every evaluated molecule consumes budget.
- Fragment constraint:
  - Enumerates and tokenizes the required MockDock scaffold fragment using InVirtuoGen's `[i*]` attachment point notation.
  - Forces the model to use the scaffold fragment prompt in the starting population.
  - Prepends the scaffold fragment to crossover prompts produced by the `GeneticPrompter` each round, ensuring 100% hard constraint satisfaction.
- Hyperparameters:
  - Standardized effective batch size to 64.
  - Set `dt=0.01`, `eta=999`, `first_pop_size=10`.
- PyTorch Lightning CUDA Deepcopy Fix:
  - Replaced `self.prior.model = copy.deepcopy(self.model.model)` in both `initialize_population` and `evolve_population` with `self.prior.model.load_state_dict(self.model.model.state_dict())` to prevent hanging/stalling issues during fine-tuning on SLURM GPU nodes.
- Warmup Initial Compounds seeding:
  - Added support for fetching and scoring the initial bioactivity compounds (warmups) at the start of the optimization loop.
  - Tokenizes warmup compounds using `decompose_smiles` and feeds them directly into the starting training/prompter population for the initial PPO update step, guiding RL toward high-scoring templates.
  - Configurable via `--n-warmup` (defaults to 25, matching the other models).
- Host GPU Detection Fix:
  - Modified `torch_cuda_is_available_host` in `run.py` to check for `CUDA_VISIBLE_DEVICES` or `nvidia-smi` when PyTorch is not installed in the host's virtual environment. This resolves a bug that caused the runner to fall back to CPU execution under SLURM.

## 2026-06-24

### InVirtuoGen (`benchmark/exps/invirtuogen/`)

- PyTorch device compatibility fix in `optimizer.py`:
  - Added `.to(prompts.device)` to `scaffold_tokens` in `prepend_scaffold_to_prompts` before performing `torch.cat` with valid tokens generated by the model. This prevents a `RuntimeError` due to tensors being on different devices (CPU vs. GPU).
- Single SMILES score type fix in `MockDockOracleAdapter`:
  - Updated the adapter's `__call__` method to return a single float instead of a list of floats when passed a single SMILES string. This resolves a `TypeError` when checking if `score > max(self.scores)` during genetic algorithm mutations.
