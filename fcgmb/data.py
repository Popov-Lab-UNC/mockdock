...
def _find_cache_directory(cache_dir: Optional[Union[str, Path]] = None) -> Optional[Path]:
    """
...
def fetch_chembl_data(
    ...
    if use_cache:
        ...
        if found_cache_dir:
            ...
            if assay_chembl_id:
                assay_cache_file = found_cache_dir / target_chembl_id / f"{document_chembl_id}_{assay_chembl_id}.csv"
                if assay_cache_file.exists():
                    try:
                        ...
                        print(f"   Warning: Could not read assay cache file {assay_cache_file}: {e}")
            ...
            if doc_cache_file.exists():
                try:
                    ...
                    if assay_chembl_id and 'assay_chembl_id' in df.columns:
                        ...
                        print(f"   Filtered document cache to assay {assay_chembl_id}: {n_before} -> {len(df)} compounds")
...
# Fallback message
            ...
            print(f"   Will attempt API call...")
        else:
            # Cache directory doesn't exist - suggest building cache
            if cache_only:
                raise RuntimeError(
                    f"Cache-only mode enabled but cache directory not found.\n"
                    f"Build the cache first:\n"
                    f"python fcgmb/pipeline/cache_chembl_data.py --config-dir configs --cache-dir data/chembl_cache"
                )
            print(f"   Cache directory not found. To avoid API rate limiting, build cache first:")
            print(f"   python fcgmb/pipeline/cache_chembl_data.py --config-dir configs --cache-dir data/chembl_cache")
    ...
    try:
        ...
        try:
            ...
            if "500" in error_str or "unavailable" in error_str or "error getting schema" in error_str:
        ...
        raise RuntimeError(
            f"ChEMBL API is currently unavailable. Please try again later or use a local CSV file.\n"
            f"To avoid API issues, build the cache first:\n"
            f"python fcgmb/pipeline/cache_chembl_data.py --config-dir configs --cache-dir data/chembl_cache"
        )
    ...
    write_cache_dir = Path(cache_dir) if cache_dir and Path(cache_dir).exists() else _find_cache_directory(None)
    ...
    if write_cache_dir is not None:
        ...
        if assay_chembl_id:
            cache_file = write_cache_dir / target_chembl_id / f"{document_chembl_id}_{assay_chembl_id}.csv"
