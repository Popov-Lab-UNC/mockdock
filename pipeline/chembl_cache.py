...
def get_cache_path(cache_dir: Union[str, Path], target_id: str, document_id: str, assay_id: str) -> Path:
...
def write_assay_cache(cache_dir: Union[str, Path], target_id: str, document_id: str, assay_id: str, df: pl.DataFrame):
    """Write raw bioactivity data for a specific assay to cache."""
    path = get_cache_path(cache_dir, target_id, document_id, assay_id)
    df.write_csv(path)

def read_assay_cache(cache_dir: Union[str, Path], target_id: str, document_id: str, assay_id: str) -> Optional[pl.DataFrame]:
...
def is_assay_cached(cache_dir: Union[str, Path], target_id: str, document_id: str, assay_id: str) -> bool:
