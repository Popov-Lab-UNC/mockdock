...
Run from benchmark root::
    ...
    python scripts/variance/generate_crystal_ligand_mapping.py
    python scripts/variance/generate_crystal_ligand_mapping.py -o variance_runs/crystal_ligand_mapping.csv
"""
...
DEFAULT_BIOACTIVITY_DIR = BENCHMARK_ROOT / "src" / "mockdock" / "bioactivity_data"
...
def load_bioactivity_compounds(benchmark_name: str) -> list[tuple[str, str]]:
    """Load local benchmark bioactivity data for custom/non-ChEMBL systems."""
    source_path = DEFAULT_BIOACTIVITY_DIR / f"{benchmark_name}.csv"
    if not source_path.exists():
        return []
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Directory with benchmark YAML/TOML configs (default: {DEFAULT_CONFIGS_DIR})",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    configs = sorted(config_dir.glob("*.yaml")) + sorted(config_dir.glob("*.toml"))
    if not configs:
        print(f"ERROR: No YAML/TOML configs found in {config_dir}", file=sys.stderr)
        sys.exit(1)
    ...
    for config_path in configs:
        ...
        bioactivity_compounds = load_bioactivity_compounds(benchmark_name)
        if crystal_smiles and bioactivity_compounds:
            best_local_id, best_sim = find_best_similarity_match(
                crystal_smiles, bioactivity_compounds, fpgen
            )
            if best_local_id is not None and best_sim >= args.min_cache_similarity:
                chembl_id = best_local_id
                cache_message = f" [local bioactivity match, sim={best_sim:.3f}]"
            elif best_local_id is not None and chembl_id is None:
                chembl_id = best_local_id
                cache_message = f" [local bioactivity fallback below threshold: {best_local_id} sim={best_sim:.3f}]"
