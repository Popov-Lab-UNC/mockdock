...
#SBATCH --output=logs/pptt_models_%j.log
#SBATCH --error=logs/pptt_models_%j.err
...
mockdock_setup_root
mockdock_slurm_header "PptT model suite"

CLIP_REWARD_UPPER_BOUND="${CLIP_REWARD_UPPER_BOUND:-${1:-false}}"
case "${CLIP_REWARD_UPPER_BOUND}" in
    true|TRUE|1|yes|YES)
        CLIP_FLAG="--clip-reward-upper-bound"
        DEST_ROOT="${DEST_ROOT:-${BENCHMARK_DIR}/exps_upperbound}"
        TMP_ROOT="${TMP_ROOT:-${BENCHMARK_DIR}/_pptt_staging/upperbound}"
        ;;
    false|FALSE|0|no|NO|"")
        CLIP_FLAG="--no-clip-reward-upper-bound"
        DEST_ROOT="${DEST_ROOT:-${BENCHMARK_DIR}/exps}"
        TMP_ROOT="${TMP_ROOT:-${BENCHMARK_DIR}/_pptt_staging/no_upperbound}"
        ;;
    *)
        echo "CLIP_REWARD_UPPER_BOUND must be true or false, got: ${CLIP_REWARD_UPPER_BOUND}"
        exit 2
        ;;
esac
...
if cfg.get("low_score") is None or cfg.get("high_score") is None:
    raise SystemExit(
        "PptT low_score/high_score are not set. Run variance first and fill thresholds before model runs."
    )
...
echo "Clip reward upper bound: ${CLIP_REWARD_UPPER_BOUND}"
...
run_acegen_model() {
    ...
    mockdock_activate_venv /work/users/s/h/shuhang/acegen-open/.venv
    for run_idx in $(seq 1 "${NUM_RUNS}"); do
        local run_tag run_seed staged_parent
        run_tag=$(printf "r%02d" "${run_idx}")
        run_seed=$((BASE_SEED + run_idx - 1))
        staged_parent="${TMP_ROOT}/${model}/run_${BASE_TS}_${run_tag}"
...
run_libinvent() {
    ...
    mockdock_activate_venv /work/users/s/h/shuhang/REINVENT4/.venv
    for run_idx in $(seq 1 "${NUM_RUNS}"); do
        local run_tag run_seed staged_parent
        run_tag=$(printf "r%02d" "${run_idx}")
        run_seed=$((BASE_SEED + run_idx - 1))
...
run_genmol() {
    ...
    module load autodock-gpu
    mockdock_activate_venv "${BENCHMARK_DIR}/.venv"
    for run_idx in $(seq 1 "${NUM_RUNS}"); do
        local run_tag run_seed staged_parent
        run_tag=$(printf "r%02d" "${run_idx}")
        run_seed=$((BASE_SEED + run_idx - 1))
...
mockdock_slurm_footer "PptT model suite"
