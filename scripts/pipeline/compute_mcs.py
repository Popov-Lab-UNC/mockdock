...
def compute_mcs(
    ...
    try:
        ...
        return Chem.MolFragmentToSmiles(
            ref_mol, atomsToUse=match_atoms, canonical=True, isomericSmiles=False
        )
...
def merge_mcs_results(mcs_results_csv: str, mapping_pattern: str, output_csv: str):
    ...
    for mapping_file in mapping_files:
        try:
            ...
            if (
                "document_chembl_id" not in mapping_df.columns
                or "assay_chembl_id" not in mapping_df.columns
                or "mcs_smiles" not in mapping_df.columns
            ):
    ...
    if dfs:
        ...
        merged_df = merged_df.drop_duplicates(
            subset=["document_chembl_id", "assay_chembl_id"], keep="last"
        )
...
def main():
    ...
    unique_pairs = df[
        ["document_chembl_id", "assay_chembl_id", "target_chembl_id"]
    ].drop_duplicates()
    ...
    if output_path.exists():
        try:
            ...
            if (
                "document_chembl_id" in existing_df.columns
                and "assay_chembl_id" in existing_df.columns
            ):
