...
def run_docking_workflow(
    ...
    if stage in ["all", "retrieve"]:
        try:
            ...
            config_parent = (
                Path(config_file_path).resolve().parent if config_file_path else Path.cwd()
            )
            if ligand_csv_path:
                resolved_ligand_csv = Path(cast(str, ligand_csv_path))
                if not resolved_ligand_csv.is_absolute():
                    resolved_ligand_csv = (config_parent / resolved_ligand_csv).resolve()
            else:
                benchmark_key = benchmark_name
                if not benchmark_key and config_file_path:
                    benchmark_key = Path(config_file_path).stem
                packaged_csv = (
                ...
                resolved_ligand_csv = packaged_csv if benchmark_key and packaged_csv.exists() else None

            if resolved_ligand_csv:
                df = pl.read_csv(resolved_ligand_csv)
...
def main():
    parser = ArgumentParser(description="Run Docking Workflow from YAML or TOML configuration")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
        help="Path to workflow configuration file (.yaml or .toml).",
    )
    parser.add_argument(
        "--stage",
        type=str,
