...
def fetch_assays_for_target(
    target_chembl_id: str, min_compounds: int = 20, cache_dir: str = "data/chembl_cache"
):
    """
...
def find_matching_compounds(
    crystal_fp, doc_smiles_list: list, fpgen, similarity_threshold: float = 1.0
):
    """
...
def main():
    ...
    if args.end:
        ...
        print(
            f"Processing subset: indices {args.start} to {args.end} ({len(unique_targets)} targets)"
        )
    ...
    for target_id in tqdm(unique_targets, desc="Processing targets"):
        ...
        if use_sqlite:
            ...
            assays = fetch_assays_for_target(
                target_id, min_compounds=args.min_compounds, cache_dir=args.cache_dir
            )
