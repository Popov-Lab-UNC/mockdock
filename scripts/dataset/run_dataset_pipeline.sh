...
#       bash scripts/dataset/run_dataset_pipeline.sh step1  # Fetch ChEMBL targets
#       bash scripts/dataset/run_dataset_pipeline.sh step2  # Map to PDB (submit array job)
#       bash scripts/dataset/run_dataset_pipeline.sh step3  # Filter drug-like
#       bash scripts/dataset/run_dataset_pipeline.sh step4  # Find matching documents (submit array job)
#       bash scripts/dataset/run_dataset_pipeline.sh step4b # Compute MCS for documents (submit array job)
#       bash scripts/dataset/run_dataset_pipeline.sh step5  # Generate configs
#
# =============================================================================
...
SBATCH_EXPORT="ALL,PROJECT_ROOT"
if [[ -n "${CHEMBL_SQLITE_PATH:-}" ]]; then
    SBATCH_EXPORT="${SBATCH_EXPORT},CHEMBL_SQLITE_PATH"
fi
...
case "$1" in
    sequential)
        ...
        if [[ -n "${CHEMBL_SQLITE_PATH:-}" ]]; then
            python scripts/dataset/fetch_chembl_targets.py --chembl-sqlite "${CHEMBL_SQLITE_PATH}"
        else
            python scripts/dataset/fetch_chembl_targets.py
        fi
        python scripts/dataset/map_pdb_ligands.py
        python scripts/dataset/filter_druglike.py
        if [[ -n "${CHEMBL_SQLITE_PATH:-}" ]]; then
            python scripts/dataset/find_matching_documents.py --chembl-sqlite "${CHEMBL_SQLITE_PATH}"
        else
            python scripts/dataset/find_matching_documents.py
        fi
    ...
    slurm)
        ...
        JOB1=$(sbatch --parsable --export=${SBATCH_EXPORT} scripts/dataset/slurm_01_fetch_targets.sbatch)
        echo "Step 1 submitted: Job $JOB1"
        
        # Step 2: Map to PDB (depends on step 1)
        JOB2=$(sbatch --parsable --export=${SBATCH_EXPORT} --dependency=afterok:$JOB1 scripts/dataset/slurm_02_map_pdb.sbatch)
        echo "Step 2 submitted: Job $JOB2 (array)"
        
        # Step 3: Filter (depends on all of step 2)
        JOB3=$(sbatch --parsable --export=${SBATCH_EXPORT} --dependency=afterok:$JOB2 scripts/dataset/slurm_03_filter.sbatch)
        echo "Step 3 submitted: Job $JOB3"
        
        # Step 4: Find documents (depends on step 3)
        JOB4=$(sbatch --parsable --export=${SBATCH_EXPORT} --dependency=afterok:$JOB3 scripts/dataset/slurm_04_find_docs.sbatch)
        echo "Step 4 submitted: Job $JOB4 (array)"
        
        # Step 4b: Compute MCS (depends on all of step 4)
        JOB4B=$(sbatch --parsable --export=${SBATCH_EXPORT} --dependency=afterok:$JOB4 scripts/dataset/slurm_04b_mcs.sbatch)
        echo "Step 4b submitted: Job $JOB4B (array)"
        
        # Step 5: Generate configs (depends on all of step 4b)
        JOB5=$(sbatch --parsable --export=${SBATCH_EXPORT} --dependency=afterok:$JOB4B scripts/dataset/slurm_05_generate_configs.sbatch)
    ...
    step2)
        ...
        sbatch --export=${SBATCH_EXPORT} scripts/dataset/slurm_02_map_pdb.sbatch
        ;;
    
    step3)
        echo "Running Step 3: Filter drug-like..."
        sbatch --export=${SBATCH_EXPORT} scripts/dataset/slurm_03_filter.sbatch
        ;;
    
    step4)
        echo "Submitting Step 4: Find matching documents (SLURM array)..."
        sbatch --export=${SBATCH_EXPORT} scripts/dataset/slurm_04_find_docs.sbatch
        ;;
    
    step4b)
        echo "Submitting Step 4b: Compute MCS for documents (SLURM array)..."
        sbatch --export=${SBATCH_EXPORT} scripts/dataset/slurm_04b_mcs.sbatch
        ;;
    
    step5)
        echo "Running Step 5: Generate configs..."
        sbatch --export=${SBATCH_EXPORT} scripts/dataset/slurm_05_generate_configs.sbatch
        ;;
